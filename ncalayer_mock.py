...
class NCALayerMock:
    ...
    async def handle_basics_sign(self, args: Dict) -> Dict:
        ...
        if isinstance(data, list):
            # Мультиподписание
            results = [self.generate_fake_signature(str(item), format_type) for item in data]
    ...
    async def handle_common_utils_module(self, method: str, args: list) -> Dict:
        ...
        if method == "getVersion":
            return {
                "result": {"version": "3.0.0.0"}
            }
        
        elif method == "getActiveTokens":
            return {
                "code": "200",
                "responseObject": list(self.storages.keys())
            }
            ...
            return {
                "code": "200", 
                "responseObject": self.generate_fake_certificate()
            }
            ...
            return {
                "code": "200",
                "responseObject": f"Locale changed to {locale_id}"
            }
    ...
    async def handle_kaztoken_module(self, method: str, args: list) -> Dict:
        ...
        if method == "getVersion":
            return {
                "result": {"version": "2.1.0.0", "type": "KAZTOKEN_DESKTOP"}
            }
        else:
            return {
                "code": "404",
                "message": f"KAZTOKEN method {method} not supported"
            }
